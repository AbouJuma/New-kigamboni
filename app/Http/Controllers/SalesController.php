...
// use SimpleSoftwareIO\QrCode\QrCodeFacade as QrCode;
...
class SalesController extends BaseController
{
  ...
  public function store(Request $request)
        ...
        \DB::transaction(function () use ($request, $validated) {
            ...
            foreach ($request['details'] as $detail) {
                ...
                $orderDetails[] = [
                    'date' => $request->date,
                    'sale_id' => $order->id,
                    'sale_unit_id' => $detail['sale_unit_id'] ?? null,
    ...
    //-------------- Print Invoice ---------------\\
  public function get_vfd_access_token()
{
    try {
        $cacheKey = 'vfd_access_token';
        $accessToken = Cache::get($cacheKey);

        if (!$accessToken) {
            // Fetch new token
            $tokenResponse = Http::asForm()->post('https://app.webvfd.co.tz/v1/gateway/get-token', [
                'username' => 'GODSONLEMA',
                'password' => '123456',
                'companyID' => 'ATGRU7',
                'serial_number' => '10TZ109239'
            ]);
            ...
            Cache::put($cacheKey, $newToken, $expiresAt); // Store token in cache
    ...
    public function get_vfd_comp_details()
{
    try {
        $companyCacheKey = 'vfd_company_details';
        $companyData = Cache::get($companyCacheKey);
        ...
        // Fetch access token
        $accessTokenResponse = $this->get_vfd_access_token();
        $responseData = $accessTokenResponse->getData(true);
        ...
        $registrationResponse = Http::asForm()->withHeaders([
            'Authorization' => "Bearer {$access_token}",
            'Accept' => 'application/json'
        ])->post('https://app.webvfd.co.tz/v1/gateway/comp-registration', [
            "companyID" => "ATGRU7",
            "serial_number" => "10TZ109239",
            "TIN" => 107072950
        ]);
        ...
        $registrationData = $registrationResponse->json()['response'];

        if (isset($registrationData['AUTH_KEY'], $registrationData['COMPANY'])) {
            $authKey = $registrationData['AUTH_KEY'];
            $companyDetails = $registrationData['COMPANY'];
            ...
            $companyData = [
                ...
                'company' => [
                    ...
                    'mobile' => $companyDetails['MOBILE'] ?? 'N/A',
                    'TIN' => $companyDetails['TIN'] ?? 'N/A',
                    'VRN' => $companyDetails['VRN'] ?? 'N/A',
                    'serial_number' => $companyDetails['SERIAL_NO'] ?? 'N/A',
                    'UIN' => $companyDetails['UIN'] ?? 'N/A',
                    'tax_office' => $companyDetails['TAX_OFFICE'] ?? 'N/A'
                ]
            ];

            Cache::put($companyCacheKey, $companyData, now()->addDay()); // Cache for 24 hours
    ...
    public function post_vfd_sales_details(Sale $order, array $orderDetails)
{
    try {
        \Log::info("Details", $orderDetails);
        
        // Fetch from cache instead of making API calls
        $access_token = Cache::get('vfd_access_token');
        $companyData = Cache::get('vfd_company_details');
        ...
        if (!$companyData) {
            return response()->json(['error' => 'Company details not found'], 400);
        }

        $authKey = $companyData['auth_key'];
        ...
        $salesData = [
            ...
            'AUTH_KEY' => $authKey,
            'TIN' => 107072950,
            'SERIALNUMBER' => '10TZ109239',
            'SALES' => [
                [
                    'REFERENCE_NO' => $order->Ref,
                    'CUSTOMER' => [
                        'CUSTNAME' => $order->client_name,
        ...
        $receiptResponse = Http::withHeaders([
            'Authorization' => "Bearer {$access_token}",
            'Accept' => 'application/json',
        ])->post('https://app.webvfd.co.tz/v1/gateway/submit', $salesData);

            \Log::info("VFD Response", $receiptResponse->json());

            if (!$receiptResponse->successful()) {
                throw new Exception('Failed to submit receipt: ' . $receiptResponse->body());
            }

            $qrCode = new Generator();
            $qrCodeImage = $qrCode->format('svg')->size(200)->generate($receiptDetails['COUNTER']['QR_CODE'] ?? 'n/a');

            $receiptDetails = $receiptResponse->json();
            return response()->json([
                // 'accessTokenResponse'=>$accessTokenResponse,
                'receipt' => [
                    'fiscal_code' => $receiptDetails['RECEIPT']['FISCAL_CODE'] ?? 'n/a',
                    'receipt_no' => $receiptDetails['RECEIPT']['RECEIPT_NO'] ?? 'n/a',
                    'z_no' => $receiptDetails['RECEIPT']['Z_NO'] ?? 'n/a',
                    'date' => $receiptDetails['RECEIPT']['DATE'] ?? 'n/a',
                    'time' => $receiptDetails['RECEIPT']['TIME'] ?? 'n/a',
                ],
                'counter' => [
                'gc' => $receiptDetails['COUNTER']['GC'] ?? 'n/a',
                'dc' => $receiptDetails['COUNTER']['DC'] ?? 'n/a',
                'qr_code' => $receiptDetails['COUNTER']['QR_CODE'] ?? 'n/a',
                'qr_code_image' => base64_encode(
                 QrCode::format('svg')->size(90)->generate($receiptDetails['COUNTER']['QR_CODE'] ?? 'n/a')
    ...
    public function Print_Invoice_POS(Request $request, $id)
    {
        ...
        $comp_details = $this->get_vfd_comp_details();
        $receipt_data = $this->post_vfd_sales_details($sale, $details);
        ...
        return response()->json([
            ...
            'sale' => $item,
            'details' => $details,
            'comp_details' => $comp_details->original,
            'receipt_data' => $receipt_data->original
        ]);
    ...
    public function get_today_sales(Request $request){
        ...
        $data['total_sales_amount'] = Sale::where('deleted_at', '=', null)
        ->whereDate('date', $today)
        ->sum('GrandTotal');

        $data['total_amount_paid'] = Sale::where('deleted_at', '=', null)
        ->whereDate('date', $today)
        ->sum('paid_amount');

        $data['total_cash'] = PaymentSale::where('deleted_at', '=', null)
        ->whereDate('date', $today)
        ->where('Reglement', 'Cash')
        ->sum('montant');

        $data['total_credit_card'] = PaymentSale::where('deleted_at', '=', null)
        ->whereDate('date', $today)
        ->where('Reglement', 'credit card')
        ->sum('montant');

        $data['total_cheque'] = PaymentSale::where('deleted_at', '=', null)
        ->whereDate('date', $today)
        ->where('Reglement', 'cheque')
        ->sum('montant');
