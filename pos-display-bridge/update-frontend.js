/**
 * Updated Customer Display Bridge Integration
 * Uses PHP backend instead of local Node.js service
 */

class CustomerDisplayBridge {
  constructor() {
    this.apiBase = 'https://client.ecofieldgroup.com/delight/pos-display-bridge/display-bridge.php';
    this.enabled = true;
    this.lastTotal = 0;
    ...
    // Check if service is available on initialization
  ...
  async checkServiceAvailability() {
    try {
      ...
      this.enabled = data.success;
      console.log('PHP display service available:', data);
      return true;
    } catch (error) {
      console.log('PHP display service not available:', error.message);
      this.enabled = false;
      return false;
    }
  }
  ...
  async sendTotal(total) {
    ...
    // Only send if total changed significantly
